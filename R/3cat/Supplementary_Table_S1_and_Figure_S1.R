...
rho_dt = read_csv("../../results/3cat/rhomat_from_superpop.csv") 
rho_dt
...
rho_dt_main = rho_dt %>% 
  ...
  select(beta_X_list,latent_ICC,ordinal_dist_type,gamma0,gamma1,#mean_rank_icc,
         rho11:rho12,bin_rho0, bin_rho01 ) %>%
...
rho_dt_main_fmt %>% write_csv("../../manuscript_tables_and_figures/Supplementary/Supplementary_Table_S1.csv")


#### Supplementary Figure S1
rho_heatmap = rho_dt_main %>% select(-contains('gamma'),#-mean_rank_icc,
                                     -contains("bin_rho"))
rho_heatmap 
