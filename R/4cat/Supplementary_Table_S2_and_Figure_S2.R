...
rho_dt = read_csv("../../results/4cat/rhomat_from_superpop_4cat.csv")
rho_dt

icc_dt = read_csv("../../results/4cat/rankICCs_from_superpop_4cat.csv")
icc_dt
...
rho_dt_main = rho_dt %>% 
  ...
  select(beta_X_list,latent_ICC,ordinal_dist_type,contains("gamma"),mean_rank_icc,
         rho11:rho22,bin_rho0, bin_rho01) %>%
...
rho_dt_main_fmt = rho_dt_main %>% filter(beta_X_list==log(1.5)) %>% 
  transmute(#beta_X= factor(beta_X_list,levels=c(log(1.5),log(2)),
    ...
    rho11,rho12,rho22,rho_Bin2 = bin_rho01) %>%
  mutate_at(vars(gamma0:rho_Bin2),~ sprintf("%.4f", .x))

#### Supp Table S2
rho_dt_main_fmt %>% write_csv("../../manuscript_tables_and_figures/Supplementary/Supplementary_Table_S2.csv")
...
plot_rho36_by_or <- function(rho_heatmap, OR_target = 1.5,
                             ord_levels = c("Bell","Common","Rare","U-shape","High01low2","Even"),
...
p_or15_hori <- plot_rho36_by_or(rho_heatmap, OR_target = 1.5,
                                ord_levels = c("Bell1", "Bell2","Common", "Rare", "U-shape", "Even"))
p_or15_hori
