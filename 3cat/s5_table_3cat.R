...
out_full <- read_csv("power_comparison_z_u_ricc_022526.csv") %>%
...
out_full %>% write_csv("table_3cat.csv")
...
df_fmt <- out_full %>%
  ...
  ungroup()
df_fmt %>% write_csv("table_3cat_fmt.csv")
...
out_full <- read_csv("power_comparison_z_u_ricc_022526.csv") %>%
  filter(  number_clusters < 40) %>%
  filter( !(cluster_sizes == 50 & beta_X_list < log(2))) %>%
  select(
    number_clusters, cluster_sizes, beta_X_list, ordinal_dist_type, latent_ICC, 
    Empirical_adj_model_based_K_ncat, Empirical_noadj_model_based,
    GEE_exch, GEE_bin1, GEE_bin2,
    WH_DE_latent_icc, WH_DE_rank_icc, WH
  ) %>%
  ...
  mutate(across(Empirical:WH, function(x) x * 100)) %>% 
  arrange(`#Clusters`,Size,`Ordinal Dist.`,`Latent ICC`)
out_full %>% write_csv("table_3cat_supp.csv")


est_cols <- c("WH-DE-Latent","WH-DE-Rank","GEE-Binary1","GEE-Binary2","GEE-Ordinal","WH")

df_fmt <- out_full %>%
  mutate(across(c(Empirical, all_of(est_cols)), ~ round(.x, 1))) %>%  # if your CSV is 0-1
  rowwise() %>%
  mutate(
    min_diff = min(abs(c_across(all_of(est_cols)) - Empirical)),
    across(all_of(est_cols),
  ...
  ungroup()
df_fmt %>% write_csv("table_3cat_supp_fmt.csv")
