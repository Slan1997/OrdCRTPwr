#### update: need to record the mean beta and se beta, before & after sandwich 
library(dplyr)
library(rms)
library(readr)
library(ordinal)
...
source("~/Composite_Endpoints/proj2/rho_mat_update.R")
source("~/Composite_Endpoints/proj2/sandwich_var.R")

result_path = "~/Composite_Endpoints/proj2_setup_011926/results/mygee_emp_power_K_2/"

print_info = F
scenarios_addrhomat = read_csv("~/Composite_Endpoints/proj2_setup_011926/results/emp_power_scenarios_add_ushape020426.csv")
...
while (length(pvals) < target_success && n_tried < max_tries) {
  ...
  for (k in 1:max_iter) {
    ...
    treat01 = c(0,1)
    cum1_est = inv_logit(gamma1_est + beta_treat_est * treat01)
    cum2_est = inv_logit(gamma2_est + beta_treat_est * treat01)
